$(function ()
{
	...
	var chkShowTemplates = $("#show_templates");
	chkShowTemplates.on("change", function ()
	{
		if (this.checked)
		{
			loadProject(project.id + 1, true);
		} else
		{
			loadProject(project.id + 1);
		}
	...
	function loadProject(projectId, showTemplates)
	{
	...
	function buildTemplatesGraphics(projectData)
	{
		...
		// Load graph data
		d3.json("./api/graph/{0}/templates".f(projectData.id + 1), function (graph)
		{
			// Clears canvas data excepts <defs>
			canvas.selectAll("g.node").remove();
			canvas.selectAll("line.link").remove();
			...
			document.force = d3.layout.force()
				...
				.linkDistance(function (d)
				{
					if (d.source.leaf || d.target.leaf)
					{
						return 1.5;
					}
					return 0.5;
